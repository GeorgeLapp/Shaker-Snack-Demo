...
const PORT_PATH =
  ...
  process.env.PORT_PATH ||
  "/dev/ttyS4";
const BAUD_RATE = Number(process.env.PAYMENT_BAUD_RATE || process.env.BAUD_RATE || 9600);
const CASHLESS_NUMBER = Number(process.env.PAYMENT_CASHLESS_NUMBER || 1);
const DEBUG = process.env.PAYMENT_DEVICE_DEBUG
  ? ["1", "true", "yes", "on"].includes(String(process.env.PAYMENT_DEVICE_DEBUG).toLowerCase())
  : true;
...
async function main() {
  ...
  console.log("[CFG]", { portPath: PORT_PATH, baudRate: BAUD_RATE, cashlessNumber: CASHLESS_NUMBER, debug: DEBUG });
